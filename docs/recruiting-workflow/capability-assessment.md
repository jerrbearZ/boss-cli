...
| Send standardized reply | Implemented via browser-backed adapter | `boss recruiter reply-browser <friendId> "message" -y` | The old `boss recruiter reply` path points at a fast-reply HTTP endpoint and failed for normal typed chat. The new path uses Boss Web's own chat context and verifies through the latest-message API. |
...
poll recruiter inbox
  ...
  -> send approved standardized reply with WeChat guidance via browser-backed Boss Web adapter
...
- Some actions need fresh browser-derived cookies, `__zp_stoken__`, or an initialized Boss Web browser context.
